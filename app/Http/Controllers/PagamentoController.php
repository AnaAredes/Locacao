...
class PagamentoController extends Controller
{
    ...
    */
    private function processarDadosReserva(Request $request, $bem_id): array
    {
        ...
        $diarias = $data_inicio->diffInDays($data_fim);

        // Verifica disponibilidade
        $disponivel = $this->disponibilidadeFinder->verificaDisponibilidade($bem_id, $data_inicio, $data_fim);
        if (!$disponivel) {
            return ['erro' => true, 'mensagem' => 'Casa indisponível para as datas selecionadas.'];
        }

        // Busca o bem
        $bem = BemLocavel::find($bem_id);
        if (!$bem) {
            return ['erro' => true, 'mensagem' => 'Bem não encontrado.'];
        }
